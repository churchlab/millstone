...
# This function is exactly the same as above, but since the funcs
# these functions decorate are also decorated the celery's @task decorateor
# the wrapper returned must be different or celery will only make one task
# for both of the decorated funcs
def report_failure_stats_2(file_name):
    """Decorator that writes to file the traceback and exception of the
    decorated function, which must have one argument that is an instance
    of the ExperimentSampleToAlignment model class that it will set the
    assembly status of to failed
